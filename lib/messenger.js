...
//   - AI reply via Groq (same RESTAURANT_CONTEXT, intent detection)
...
const GROQ_API_KEY = process.env.GROQ_API_KEY || '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';
...
// ---- AI reply generation (Groq gpt-4o) --------------------------------
...
async function sendMessengerMessage(pageToken, recipientId, message) {
  ...
  try {
    var graphBase = /^IG/.test(pageToken) ? 'https://graph.instagram.com' : 'https://graph.facebook.com';
    var url = graphBase + '/v22.0/me/messages?access_token=' + encodeURIComponent(pageToken);
...
async function fetchUserProfile(pageToken, userId) {
  ...
  try {
    var graphBase = /^IG/.test(pageToken) ? 'https://graph.instagram.com' : 'https://graph.facebook.com';
    var fields = /^IG/.test(pageToken) ? 'name,username' : 'name,first_name,last_name,username';
    var url = graphBase + '/v22.0/' + encodeURIComponent(userId) +
      '?fields=' + fields +
